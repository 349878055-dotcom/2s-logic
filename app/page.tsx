...
import { PRESENTATION_SCRIPTWORKBENCH_VIEWPORT } from "@/workflows/presentation/scriptworkbench";
...
export default function ScriptWorkbench() {
  // ── 路由状态 ──────────────────────────────────────────
  const [activeNav, setActiveNav] = useState("剧本区"); // 默认进剧本区；侧栏项在 constants NAV_* 中均为 open 时可切换
  ...
  const handleNavClick = (label: string) => {
    ...
    // 侧栏 disabled 由 NAV_* 的 status === "building" 控制；全开时为 open
  };
  ...
  const VIEW_HEADERS: Record<string, { icon: React.ElementType; iconColor: string; title: string; sub: string; extra?: React.ReactNode }> = {
    ...
    "剧本区": {
      icon: Code2,
      iconColor: "#a78bfa",
      title: PRESENTATION_SCRIPTWORKBENCH_VIEWPORT.title,
      sub: PRESENTATION_SCRIPTWORKBENCH_VIEWPORT.subtitle,
    },
  };
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#0a0a0a] text-[#d1d5db]">
      ...
      <main className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        {activeNav === "剧本区" && (
          <Suspense fallback={<div className="flex min-h-0 flex-1 items-center justify-center text-sm text-zinc-500">加载剧本区…</div>}>
